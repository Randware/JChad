    ...
    public class Bubble extends Group {
        ...
        public Bubble(String text, String meta, double maxWidth) {
            super();
            init(0, 0, text, meta, maxWidth);
        }
         ...
         * @param text
         */
        public Bubble(String text, LocalDateTime meta, boolean quadratic) {
            super();
            if(quadratic == true) {
                init(0, 0, text, String.valueOf(meta), null);
        ...
        public Bubble(int x, int y, String text, String meta) {
            super();
            init(x, y, text, meta, null);
        }

        private void init(int x, int y, String text, String meta, Double maxWidth) {
            // temp for text
            Text temp = new Text(text);
            temp.setFont(textFont);
            if (maxWidth != null) {
                temp.setWrappingWidth(maxWidth);
            }
            int textW = (int) temp.getLayoutBounds().getWidth();
            int textH = (int) temp.getLayoutBounds().getHeight();

            int w = textW + p * 2 + s * 2; // Initial width calculation
            int h = textH + p * 2;
            temp = null;
            ...
            // Check if calculated width is less than the minimum width
            if (w < 170) {
                w = 170; // Set width to minimum width
            }
